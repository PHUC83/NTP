	...
	float sx, float sy, float sw, float sh)
{
	...
	vertices.push_back({ { x0, y0 , 0 }, { r, g, b, a }, { u0, v0 } });	//頂点0
	vertices.push_back({ { x1, y1 , 0 }, { r, g, b, a }, { u1, v1 } });	//頂点1
	vertices.push_back({ { x2, y2 , 0 }, { r, g, b, a }, { u2, v2 } });	//頂点2

	vertices.push_back({ { x2, y2 , 0 }, { r, g, b, a }, { u2, v2 } });	//頂点2
	vertices.push_back({ { x1, y1 , 0 }, { r, g, b, a }, { u1, v1 } });	//頂点1
	vertices.push_back({ { x3, y3 , 0 }, { r, g, b, a }, { u3, v3 } });	//頂点3
}
	...
	: max_vertices(max_sprites * 6)
{
	...
	//_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	 ...
	 hr = create_vs_from_cso(device, cso_name,
	...
	{
		const char* cso_name{ "sprite_ps.cso" };
		hr = create_ps_from_cso(device, cso_name, &pixel_shader);
		_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	//HRESULT load_texture_from_file(sprite_batch(ID3D11Device* device, const wchar_t* filename, size_t max_sprites),
